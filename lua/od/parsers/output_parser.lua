...
function M.parse_debug_output(output, filetype)
    local handler = M.line_handlers[filetype]
    if not handler then
        handler = M.default_handler
    end
    if handler then
        local ctx = { errors = {}, warnings = {}, output = output }
        for line in output:gmatch("[^\n]+") do
            handler(line, ctx)
        end
        if M.on_parse then
            M.on_parse(output, filetype, ctx.errors, ctx.warnings)
        end
        ...
        if line ~= nil then
            ...
            if filetype == "c" or filetype == "cpp" then
                ...
                local debug_tool_patterns = {
                    ...
                    -- LLDB patterns
                    { pattern = "Process %d+ stopped",                        type = "LLDB-STOPPED" },
                    { pattern = "stop reason =",                              type = "LLDB-REASON" },
                    { pattern = "error: lldb:",                               type = "LLDB-ERROR" },
                    { pattern = "error:.*unable to",                          type = "LLDB-UNABLE" },
                    { pattern = "error:.*connection",                         type = "LLDB-CONNECTION" },
                    { pattern = "Current executable set to",                  type = "LLDB-EXECUTABLE" },
                    { pattern = "lldb: warning:",                             type = "LLDB-WARNING" },
                    { pattern = "frame #%d+:",                                 type = "LLDB-FRAME" },
                    { pattern = "Connection established",                       type = "LLDB-CONNECTED" },
                    { pattern = "lldb:.*error:",                              type = "LLDB-INTERNAL-ERROR" },
                ...
                local build_error_patterns = {
                    ...
                    {
                        pattern =
                        "CMake Error: add_executable cannot create target .* because another target with the same name already exists",
    ...
    if M.on_parse then
        M.on_parse(output, filetype, errors, warnings)
    end
    return errors, warnings
end

function M.register_parser(lang, fn)
    M.line_handlers[lang] = fn
end

function M.add_error_keywords(lang, list)
    M.error_keywords[lang] = M.error_keywords[lang] or {}
    for _, kw in ipairs(list) do
        table.insert(M.error_keywords[lang], kw)
    end
end

function M.add_warning_patterns(lang, list)
    M.warning_patterns[lang] = M.warning_patterns[lang] or {}
    for _, p in ipairs(list) do
        table.insert(M.warning_patterns[lang], p)
    end
end

function M.add_runtime_patterns(lang, list)
    M.runtime_patterns[lang] = M.runtime_patterns[lang] or {}
    for _, p in ipairs(list) do
        table.insert(M.runtime_patterns[lang], p)
    end
end

function M.set_default_parser(fn)
    M.default_handler = fn
end

function M.set_on_parse(fn)
    M.on_parse = fn
end
