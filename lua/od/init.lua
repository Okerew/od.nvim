        ...
        vim.list_extend({ cmd }, args),
        {
            ...
            on_exit = function(_, code)
                ...
                if #errors > 0 then
                    ...
                    create_test_picker(errors, "Test Errors")
                elseif #warnings > 0 then
                    attach_log(warnings, full_output)
                    create_test_picker(warnings, "Test Warnings")
                else
    ...
    vim.fn.jobstart(
        vim.list_extend({ cmd }, args),
        {
            ...
            on_exit = function(_, code)
                ...
                local chunks = split_into_chunks(full_output, filetype)
                local errors, warnings = chunks_to_items(chunks, filetype, vim.fn.expand("%"))
...
function M.js_test()
    local filetype = vim.bo.filetype
    if
        filetype ~= "javascript" and filetype ~= "typescript" and filetype ~= "javascriptreact" and
        ...
        if fallbacks[cmd] then
            for _, fallback in ipairs(fallbacks[cmd]) do
                if vim.fn.executable(fallback) == 1 then
                    ...
                    elseif (cmd ~= debugger_config.test_args[1]) and (cmd == "yarn" or cmd == "npm" or cmd == "pnpm") then
    ...
    -- Check if cursor is on a specific test function
    local current_func = get_current_function()
    if current_func and (current_func:match("^test") or current_func:match("^it") or current_func:match("^describe")) then
    ...
    vim.fn.jobstart(
        vim.list_extend({ cmd }, args),
        {
            ...
            on_exit = function(_, code)
                ...
                local errors, warnings = chunks_to_items(chunks, lang, vim.fn.expand("%"))
...
function M.busted_test()
    ...
    -- Check if cursor is on a specific test function
    local current_func = get_current_function()
    if current_func and (current_func:match("^test_") or current_func:match("^it") or current_func:match("^describe")) then
    ...
    vim.fn.jobstart(
        vim.list_extend(job_cmd, args),
        {
            ...
            on_exit = function(_, code)
                ...
                if #errors > 0 then
                    create_test_picker(errors, "Busted Test Errors")
                elseif #warnings > 0 then
                    create_test_picker(warnings, "Busted Test Warnings")
                else
